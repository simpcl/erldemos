...
start() ->
    start([{port, 9999}, {name, test_gen_server}]).
...
sleep(Time) ->
    receive
    after Time ->
        error_logger:info_msg("sleep timeout ~p~n", [Time])
    end.
...
do_process(Socket) ->
    case gen_tcp:recv(Socket, 0) of
        {ok, B} ->
            error_logger:info_msg("from ~p receive ~s", [Socket, B]),
            gen_tcp:send(Socket, B),
            do_process(Socket);
        {error, Reason} ->
            error_logger:info_msg("from ~p error ~p", [Socket, Reason]),
            exit({error, closed})
    end.
